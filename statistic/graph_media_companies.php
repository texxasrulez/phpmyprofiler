...
if (
    $pmp_thumbnail_cache &&
    is_file("../cache/graph_media_companies_" . $graph_lang . ".png")
) {
    $filename = "../cache/graph_media_companies_" . $graph_lang . ".png";
    ...
    $query =
        'SELECT name AS company, data AS count FROM pmp_statistics WHERE type = \'company_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
    ...
    if (mysql_num_rows($result) == 0) {
        ...
        $graph = new Graph(
            550,
            160,
            "graph_media_companies_" . $graph_lang . ".png",
        );
