...
if (
    $pmp_thumbnail_cache &&
    is_file("../cache/graph_localities_" . $graph_lang . ".png")
) {
    $filename = "../cache/graph_localities_" . $graph_lang . ".png";
    ...
    $query =
        'SELECT name AS locality, data AS count FROM pmp_statistics WHERE type = \'localities\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
    ...
    if ($rows == 0) {
        ...
        $graph = new Graph(
            550,
            12 * $rows + 40,
            "graph_localities_" . $graph_lang . ".png",
        );
