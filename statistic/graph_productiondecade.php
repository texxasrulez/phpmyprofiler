...
if (
    $pmp_thumbnail_cache &&
    is_file("../cache/graph_productiondecade_" . $graph_lang . ".png")
) {
    $filename = "../cache/graph_productiondecade_" . $graph_lang . ".png";
    ...
    $query =
        'SELECT name AS year, data AS count FROM pmp_statistics WHERE type = \'decade\' GROUP BY year';
    $result = dbexec($query);
    ...
    if ($rows == 0) {
        ...
        $graph = new Graph(
            550,
            12 * $rows + 40,
            "graph_productiondecade_" . $graph_lang . ".png",
        );
