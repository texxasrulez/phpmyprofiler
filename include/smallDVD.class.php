    ...
    function smallDVD($id)
    {
        ...
        if (isset($id)) {
            ...
            if (@mysql_num_rows($res) > 0) {
                ...
                $this->ProfileDate = strftime(
                    $pmp_dateformat,
                    strtotime($this->_db["profiletimestamp"]),
                );

                if ($this->_db["media_bluray"] == 1) {
                    $this->Media = "Blu-ray";
                } elseif ($this->_db["media_hddvd"] == 1) {
                    $this->Media = "HD DVD";
                } elseif ($this->_db["media_dvd"] == 1) {
                    $this->Media = "DVD";
                } elseif ($this->_db["media_custom"] != "") {
                ...
                $this->Edition = htmlspecialchars(
                    $this->_db["disttrait"],
                    ENT_COMPAT,
                    "UTF-8",
                );
                $this->OriginalTitle = htmlspecialchars(
                    $this->_db["originaltitle"],
                    ENT_COMPAT,
                    "UTF-8",
                );
                $this->Year = $this->_db["prodyear"];
                if ($this->_db["released"] != "0000-00-00") {
                    $this->Released = strftime(
                        $pmp_dateformat,
                        strtotime($this->_db["released"]),
                    );
                } else {
                    $this->Released = "";
                }
                $this->Length = $this->_db["runningtime"];
                $this->LengthHours = sprintf(
                    "%d",
                    $this->_db["runningtime"] / 60,
                );
                $this->LengthMins = sprintf(
                    "%02d",
                    $this->_db["runningtime"] % 60,
                );
                $this->RatingSystem = htmlspecialchars(
                    $this->_db["ratingsystem"],
                    ENT_COMPAT,
                    "UTF-8",
                );
                ...
                if ($this->_db["srp"] != "0.00") {
                    $this->Price = number_format(
                        $this->_db["srp"],
                        get_currency_digits($this->_db["srpid"]),
                        $pmp_dec_point,
                        $pmp_thousands_sep,
                    );
                } else {
                ...
                if ($this->_db["srp"] != "0.00") {
                    $this->convAvgPrice = number_format(
                        exchange(
                            $this->_db["srpid"],
                            $pmp_usecurrency,
                            $this->_db["srp"],
                        ),
                        get_currency_digits($pmp_usecurrency),
                        $pmp_dec_point,
                        $pmp_thousands_sep,
                    );
                } else {
                ...
                if ($this->_db["gift"] == 1) {
                    $this->Gift = true;
                    $sql =
                        "SELECT * FROM pmp_users WHERE user_id = " .
                        $this->_db["giftfrom"];
                ...
                $this->Easteregg = htmlspecialchars(
                    $this->_db["easteregg"],
                    ENT_COMPAT,
                    "UTF-8",
                );
                $this->LastEdited = @strftime(
                    $pmp_dateformat,
                    strtotime($this->_db["lastedit"]),
                );
                $this->WishPriority = $this->_db["wishpriority"];
                if ($this->_db["purchprice"] != "0.00") {
                    $this->PurchPrice = number_format(
                        $this->_db["purchprice"],
                        get_currency_digits($this->_db["purchcurrencyid"]),
                        $pmp_dec_point,
                        $pmp_thousands_sep,
                    );
                } else {
                ...
                if ($this->_db["purchprice"] != "0.00") {
                    $this->ConvPrice = number_format(
                        exchange(
                            $this->_db["purchcurrencyid"],
                            $pmp_usecurrency,
                            $this->_db["purchprice"],
                        ),
                        get_currency_digits($pmp_usecurrency),
                ...
                if ($this->_db["purchdate"] != "0000-00-00") {
                    $this->PurchDate = strftime(
                        $pmp_dateformat,
                        strtotime($this->_db["purchdate"]),
                    );
                } else {
                    $this->PurchDate = "";
                }
                ...
                if ($this->_db["loaned"] == 1) {
                    ...
                    $this->LoanReturn = @strftime(
                        $pmp_dateformat,
                        strtotime($this->_db["loaneddue"]),
                    );
                } else {
                    $this->Loaned = false;
                }
                if ($pmp_html_notes == true) {
                    $this->Notices = html_entity_decode(
                        $this->_db["notes"],
                        ENT_COMPAT,
                        "UTF-8",
                    );
                } else {
                    $this->Notices = htmlspecialchars(
                        $this->_db["notes"],
                        ENT_COMPAT,
                        "UTF-8",
                    );
                ...
                // Is boxset?
                $sql =
                    "SELECT * FROM pmp_boxset LEFT JOIN pmp_film ON pmp_film.id = pmp_boxset.childid
					WHERE pmp_boxset.id = '" .
                    mysql_real_escape_string($this->id) .
                    "' ORDER BY " .
                    mysql_real_escape_string($pmp_menue_childs);
                $result = dbexec($sql);
                ...
                // Or child?
                $sql =
                    'SELECT id FROM pmp_boxset WHERE childid = \'' .
                    mysql_real_escape_string($this->id) .
                    '\'';
                $result = dbexec($sql);
                if (@mysql_num_rows($result) > 0) {
                ...
                // Check for cover
                $this->frontpic = file_exists(
                    _PMP_REL_PATH . "/cover/" . $this->id . "f.jpg",
                );
                $this->backpic = file_exists(
                    _PMP_REL_PATH . "/cover/" . $this->id . "b.jpg",
                );
    ...
    function getLocationFlag()
    {
        ...
        if (empty($flag)) {
            return '<img src="' .
                ...
                "/themes/" .
                $pmp_theme .
                '/images/flags/Noflag.gif" alt="' .
                $this->Locality .
                '" style="width: 20px; height: 12px;" title="' .
                t($this->Locality) .
                '" />';
        } else {
            return '<img src="' .
    ...
    function getMediaIcon()
    {
        ...
        if ($this->Media == "DVD") {
            return '<img src="' .
                ...
                "/themes/" .
                $pmp_theme .
                '/images/additional/DVD_s.png" alt="' .
                $this->Media .
                '" />';
        } elseif ($this->Media == "Blu-ray") {
            return '<img src="' .
                _PMP_REL_PATH .
                "/themes/" .
                $pmp_theme .
                '/images/additional/BluRay_s.png" alt="' .
                $this->Media .
                '" />';
        } elseif ($this->Media == "HD DVD") {
            return '<img src="' .
                _PMP_REL_PATH .
                "/themes/" .
                $pmp_theme .
                '/images/additional/HDDVD_s.png" alt="' .
                $this->Media .
                '" />';
        } else {
            // Custom media
            if (isset($pmp_custom_media[$this->Media])) {
                return '<img src="' .
                    ...
                    $pmp_theme .
                    "/images/additional/" .
                    $pmp_custom_media[$this->Media]["small"] .
                    '" alt="' .
                    $this->Media .
                    '" />';
            }
