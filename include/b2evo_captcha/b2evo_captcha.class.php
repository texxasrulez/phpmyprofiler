...
class b2evo_captcha
{
    ...
    function b2evo_captcha($config, $secure = true)
    {
        ...
        if ($this->debug) {
            echo "\n<br>-b2evo-Captcha-Debug: The available GD-Library has major version " .
                $this->gd_version;
        }
        ...
        if ($handle = opendir($this->TTF_folder)) {
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                if ($file != "." && $file != "..") {
                    ...
                    if ($this->debug) {
                        echo "\n<br>-b2evo-Captcha-Debug: Found font file (" .
                            $file .
                            ")";
                    }
                }
        ...
        if (is_array($this->TTF_RANGE)) {
            if ($this->debug) {
                echo "\n<br>-b2evo-Captcha-Debug: Checking given TrueType-Array! (" .
                    count($this->TTF_RANGE) .
                    ")";
            }
            $temp = [];
            ...
            $this->TTF_RANGE = $temp;
            if ($this->debug) {
                echo "\n<br>-b2evo-Captcha-Debug: Valid TrueType-files: (" .
                    count($this->TTF_RANGE) .
                    ")";
            }
            ...
            if ($this->debug) {
                echo "\n<br>-b2evo-Captcha-Debug: Check given TrueType-File! (" .
                    $this->TTF_RANGE .
                    ")";
            }
        ...
        $this->change_TTF();
        if ($this->debug) {
            echo "\n<br>-b2evo-Captcha-Debug: Set current TrueType-File: (" .
                $this->TTF_file .
                ")";
        }
        ...
        if ($this->debug) {
            echo "\n<br>-b2evo-Captcha-Debug: Set number of noise characters to: (" .
                $this->nb_noise .
                ")";
        }
        ...
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: The counterfilename is (" .
                $this->tempfolder .
                $this->counter_filename .
                ")";
        }
        ...
        $counter = $this->txt_counter(
            $this->tempfolder . $this->counter_filename,
            true,
        );
        ...
        if ($counter !== false && $counter - $test == 1) {
            ...
            if ($this->debug) {
                echo "\n<br>-Captcha-Debug: Current counter-value is ($counter). Garbage-collector should start at (" .
                    $this->collect_garbage_after .
                    ")";
            }
            ...
            if ($counter >= $this->collect_garbage_after) {
                ...
                $this->garbage_collector_error = $this->collect_garbage()
                    ? false
                    : true;
                if ($this->debug) {
    ...
    function get_b2evo_captcha()
    {
        ...
        return str_replace($_SERVER["DOCUMENT_ROOT"], "", $this->tempfolder) .
    ...
    function make_captcha($private_key = "")
    {
        ...
        $this->lx =
            ...
            (int) (($this->maxsize + $this->minsize) / 1.5);
        $this->ly = (int) (2.4 * $this->maxsize);
        if ($this->debug) {
            echo "\n<br>-b2evo-Captcha-Debug: Set image dimension to: (" .
                $this->lx .
                " x " .
                $this->ly .
                ")";
        ...
        $this->nb_noise = $this->noise
            ? strlen($private_key) * $this->noisefactor
            : 0;
        if ($this->debug) {
            echo "\n<br>-b2evo-Captcha-Debug: Set number of noise characters to: (" .
                $this->nb_noise .
        ...
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: We allocate one color for Background: (" .
                $this->r .
                "-" .
                $this->g .
                "-" .
        ...
        if ($this->nb_noise > 0) {
            ...
            if ($this->debug) {
                echo "\n<br>-Captcha-Debug: Fill background with noise: (" .
                    $this->nb_noise .
                    ")";
            }
            ...
            // generate grid
            if ($this->debug) {
                echo "\n<br>-Captcha-Debug: Fill background with x-gridlines: (" .
                    (int) ($this->lx / (int) ($this->minsize / 1.5)) .
                    ")";
            }
            ...
            if ($this->debug) {
                echo "\n<br>-Captcha-Debug: Fill background with y-gridlines: (" .
                    (int) ($this->ly / (int) ($this->minsize / 1.8)) .
                    ")";
            }
        ...
        // generate Text
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: Fill forground with chars and shadows: (" .
                $this->chars .
                ")";
        }
        for (
            $i = 0, $x = intval(mt_rand($this->minsize, $this->maxsize));
            $i < strlen($private_key);
            $i++
        ) {
            $text = substr($private_key, $i, 1);
            $angle = intval(
                mt_rand($this->maxrotation * -1, $this->maxrotation),
            );
            $size = intval(mt_rand($this->minsize, $this->maxsize));
            $y = intval(
                mt_rand((int) ($size * 1.5), (int) ($this->ly - $size / 7)),
            );
        ...
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: Save Image with quality [" .
                $this->jpegquality .
                "] as (" .
                $this->get_filename() .
                ") returns: (" .
    ...
    function makeWebsafeColors(&$image)
    {
        ...
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: Allocate 216 websafe colors to image: (" .
    ...
    function check_captcha($correct_hash, $attempt)
    {
        ...
        if (file_exists($this->get_filename($correct_hash))) {
            $res = @unlink($this->get_filename($correct_hash))
                ? "TRUE"
                : "FALSE";
            if ($this->debug) {
                echo "\n<br>-Captcha-Debug: Delete image (" .
                    $this->get_filename($correct_hash) .
                    ") returns: ($res)";
            }
        }
    ...
    function generate_keypair()
    {
        ...
        for ($i = 0; $i < $this->chars; $i++) {
            $key .=
                $this->validchars[mt_rand(0, strlen($this->validchars) - 1)];
        }
        if ($this->case_sensitive == 0) {
            $key = strtoupper($key);
        }
        ...
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: Generate Keys, private key is: (" .
                $key .
                ")";
        }
        if ($this->debug) {
            echo "\n<br>-Captcha-Debug: Generate Keys, public key is: (" .
                $this->public_key .
                ")";
        }
        return $key;
    ...
    function collect_garbage()
    {
        ...
        if ($handle = @opendir($this->tempfolder)) {
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                if (
                    substr($file, 0, strlen($this->filename_prefix)) ==
                    $this->filename_prefix
                ) {
                    if (strrchr($file, ".") == ".jpg") {
                        $captchas++;
                        if (
                            time() - filemtime($this->tempfolder . $file) >=
                            $this->maxlifetime
                        ) {
                            $trashed++;
