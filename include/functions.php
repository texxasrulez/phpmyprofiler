...
if (!isset($pmp_theme) || trim((string) $pmp_theme) === "") {
    $pmp_theme = "default";
}
if (!isset($pmp_theme_css) || trim((string) $pmp_theme_css) === "") {
...
// Runtime error policy: quiet in production, verbose when PMP_DEBUG is enabled.
if (!defined("PMP_DEBUG")) {
    define("PMP_DEBUG", false);
}
if (PMP_DEBUG) {
    ...
    error_reporting(
        E_ALL & ~E_DEPRECATED & ~E_USER_DEPRECATED & ~E_NOTICE & ~E_STRICT,
    );
}
...
if (session_status() !== PHP_SESSION_ACTIVE) {
    ...
    if (PHP_VERSION_ID >= 70300) {
        ...
        session_set_cookie_params(
            ...
            $session_params["path"] . "; samesite=Lax",
...
if (!empty($_GET["lang_id"])) {
    ...
    $_SESSION["lang_id"] = preg_replace("/[^a-z]/i", "", $_GET["lang_id"]);
}
...
function dbconnect($dieonerror = true)
{
    ...
    if (!$db && $dieonerror) {
        ...
        echo "<html><head><title>Database Error</title><style>P,BODY{ font-family:arial,sans-serif; font-size:11px; }</style>
			...
			<form name='mysql'><textarea rows=\"15\" cols=\"45\">" .
...
function db_bind_params_ref(array &$params)
{
    $refs = [];
...
function dbexec_prepared(
    ...
    if (!$link) {
        if (!$continueonerror) {
            die("Database link not available");
        }
    ...
    if (!$stmt) {
        if (!$continueonerror) {
            echo "<strong>SQL-Statement prepare failed:</strong><br /><pre>" .
                mysqli_errno($link) .
                " - " .
                mysqli_error($link) .
                "\n\nQuery:\n$sql</pre>";
    ...
    if (!mysqli_stmt_execute($stmt)) {
        if (!$continueonerror) {
            echo "<strong>SQL execute failed:</strong><br /><pre>" .
                mysqli_stmt_errno($stmt) .
                " - " .
                mysqli_stmt_error($stmt) .
                "\n\nQuery:\n$sql</pre>";
...
function trunc($string, $length, $suffix = "...")
{
    ...
    if (strlen($string) - strlen($suffix) > $length) {
        if (
            strpos($string, "&", $length - 5) &&
            strpos($string, "&", $length - 5) < $length
        ) {
            return trunc(
                $string,
                $length +
...
function getColumns($col = "", $detail = "")
{
	...
	*/
    ...
    if (empty($col)) {
        ...
        return $columns[$col];
    } elseif ($detail == "AltOutput" && !isset($columns[$col][$detail])) {
    ...
    if ($year != "") {
        ...
        if (!$found) {
            foreach ($extensions as $ext) {
                if (
                    file_exists(
                        $pmp_dir_cast . $name . " " . $searchyear . $ext,
                    )
                ) {
                    $picname = $full_encoded . $searchyear . $ext;
                    $found = true;
...
function inccounter($filmid = "")
{
    ...
    if ($filmid != "") {
        $sql =
            'SELECT COUNT(id) AS cnt FROM pmp_counter_profil WHERE sid = \'' .
            mysql_real_escape_string($sid) .
            '\' AND NOW() < DATE_ADD(date, INTERVAL 2 HOUR) AND film_id = \'' .
        ...
        if (mysql_result($result, 0) == 0) {
            $query =
                'INSERT INTO pmp_counter_profil (date, film_id, sid) VALUES (NOW(),\'' .
                mysql_real_escape_string($filmid) .
                '\', \'' .
                mysql_real_escape_string($sid) .
                '\')';
        ...
        $query =
            'SELECT COUNT(id) AS count_all FROM pmp_counter_profil WHERE film_id = \'' .
            mysql_real_escape_string($filmid) .
            '\'';
        $result = dbexec($query);
        ...
        $query =
            'SELECT COUNT(id) AS count_today FROM pmp_counter_profil WHERE date LIKE CONCAT(CURRENT_DATE,
			\'%\') AND film_id = \'' .
            mysql_real_escape_string($filmid) .
            '\'';
        $result = dbexec($query);
        ...
        $sql =
            'SELECT COUNT(id) AS cnt FROM pmp_counter WHERE sid = \'' .
            mysql_real_escape_string($sid) .
        ...
        if (mysql_result($result, 0) == 0) {
            $query =
                'INSERT INTO pmp_counter (date, sid) VALUES ( NOW(), \'' .
                mysql_real_escape_string($sid) .
                '\')';
            $result = dbexec($query);
        }
        ...
        $query =
            'SELECT COUNT(*) AS count_today FROM pmp_counter WHERE date LIKE CONCAT(CURRENT_DATE, \'%\')';
        $result = dbexec($query);
        $today = mysql_result($result, 0, "count_today");
    }
...
function t($string, $args = [])
{
    ...
    if (empty($lang_data[$string])) {
        ...
        return strtr((string) $lang_data[$string], $args);
    }
}
...
function getLang($lang_id)
{
    ...
    $data = @file_get_contents(
        _PMP_REL_PATH . "/themes/default/locale/" . $lang_id . ".xml",
    );
    if ($data === false) {
        $data = "";
    }

    while (strpos($data, "<message") !== false) {
        $tmp = gettween($data, "<message", "</message>");
        $lang_data[gettween($tmp, "<id>", "</id>")] = gettween(
            $tmp,
            "<string>",
            "</string>",
        );
    ...
    $theme_dir = _PMP_REL_PATH . "/themes/" . $theme;
...
function getLangs()
{
    ...
    $theme = (string) $pmp_theme;
    $dir = _PMP_REL_PATH . "/themes/" . $theme . "/locale";
    if ($theme === "" || !is_dir($dir)) {
        $dir = _PMP_REL_PATH . "/themes/default/locale";
    }
...
function exchange($from, $to, $value)
{
    if ($from == "EUR") {
        ...
        $ratefrom = @mysql_result(
            dbexec('SELECT rate FROM pmp_rates WHERE id = \'' . $from . '\''),
            0,
        );
    }
    ...
    if ($to == "EUR") {
        ...
        $rateto = mysql_result(
            dbexec('SELECT rate FROM pmp_rates WHERE id = \'' . $to . '\''),
            0,
        );
    }
...
function entity_to_decimal_value($string)
{
    ...
    return preg_replace(
        "/&#38;[A-Za-z]+;/",
        " ",
        strtr((string) $string, $entity_to_decimal),
    );
}
...
function add_include_path($strIncludePath)
{
    ...
    if (function_exists("ini_set")) {
        ini_set(
            "include_path",
            ini_get("include_path") . PATH_SEPARATOR . $strIncludePath,
        );
    } else {
...
{
    $sql =
        "SELECT collection FROM `pmp_collection` WHERE collection != 'Owned' AND collection != 'Ordered' AND collection != 'Wish List'";
    $res = dbexec($sql, true);
