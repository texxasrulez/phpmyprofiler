...
function buildBanners()
{
    ...
    if ($pmp_build_banners == 2 or $pmp_build_banners == 3) {
        if (isset($pmp_banner_name) && $pmp_banner_name != "") {
            $sql =
                "SELECT user_id FROM pmp_users WHERE CONCAT(firstname,' ',lastname) = '" .
                $pmp_banner_name .
                "'";
            $res = dbexec($sql);
            if (mysql_num_rows($res) > 0) {
                ...
                $sql =
                    "SELECT id FROM pmp_events WHERE EventType = 'Watched' AND user_id = " .
                    $row->user_id .
                    " ORDER BY Timestamp DESC LIMIT 0,10";
            } else {
                $sql =
                    "SELECT id FROM pmp_events WHERE EventType = 'Watched' ORDER BY Timestamp DESC LIMIT 0,10";
            }
...
function buildBanner($sql, $title, $prefix)
{
    ...
    $backimage =
        "../themes/" .
        $pmp_theme .
        "/images/banner/background_" .
        substr($pmp_theme_css, 0, -4) .
        ".png";
    if (!is_file($backimage)) {
        $backimage =
            "../themes/" . $pmp_theme . "/images/banner/background_default.png";
    }
    ...
    for ($i = 0; $i < $rows; $i++) {
        ...
        imagecopy(
            $dst_img,
            $thumb[$i],
            intval($pos_x + (50 - $th_width) / 2),
            intval(22 + (72 - $th_height) / 2),
            0,
            0,
            50,
            $th_height,
