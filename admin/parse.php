...
if (isset($_GET["action"]) && $_GET["action"] == "delete") {
    ...
    if (file_exists("./" . basename($_GET["file"]))) {
        if (@unlink("./" . basename($_GET["file"]))) {
            $smarty->assign(
                "Success",
                basename($_GET["file"]) . " " . t("deleted."),
            );
        } else {
            $smarty->assign(
                "Error",
                basename($_GET["file"]) . " " . t('can\'t be deleted.'),
            );
        }
    } else {
        $smarty->assign(
            "Error",
            basename($_GET["file"]) . " " . t("not found."),
        );
    }
}
...
if (isset($_GET["action"]) && $_GET["action"] == "split") {
    ...
    header(
        "Location:parse.php?action=parse&empty=true&file=" .
            $files[0] .
            "&start=" .
            $start .
            "&csrf_token=" .
            rawurlencode(admin_csrf_token()),
    );
    exit();
}
...
if (isset($_GET["action"]) && $_GET["action"] == "parse") {
    ...
    if (isset($_GET["file"])) {
        $parse_file = basename((string) $_GET["file"]);
        if (!preg_match('/\\.xml$/i', $parse_file)) {
            $smarty->assign("Error", t("Invalid XML file."));
            $parse_file = "";
        }
        ...
        if ($parse_file === "") {
            ...
            if ($pmp_parser_mode != 0) {
                ...
                if (
                    mysql_num_rows(
                        dbexec("SHOW TABLES LIKE 'pmp_temptable'"),
                    ) != 0
                ) {
                    ...
                    dbexec(
                        "CREATE TABLE pmp_temptable (`id` VARCHAR(20), `type` VARCHAR(9), PRIMARY KEY (`id`) ) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;",
                    );
                }
            }
            ...
            #$Parser->lockTables();
            if ($pmp_parser_mode == 0) {
                $count += $Parser->start("./split/" . $parse_file);
            } else {
                $count += $Parser->start("./" . $parse_file);
            }
            #$Parser->unlockTables();
            dbclose();
            $splits++;
            ...
            if (!empty($files)) {
                header(
                    "Location:parse.php?action=parse&file=" .
                        $files[0] .
                        "&count=" .
                        $count .
                        "&splits=" .
                ...
                header(
                    "Location:parse.php?action=finish&count=" .
                        $count .
                        "&splits=" .
                        $splits .
                        "&start=" .
...
if (isset($_GET["action"]) && $_GET["action"] == "finish") {
    ...
    if ($pmp_parser_mode != 0) {
        $sql =
            "SELECT DISTINCT (id) FROM pmp_temptable WHERE `type` != 'unaltered'";
    } else {
    ...
    if ($pmp_parser_mode != 0) {
        ...
        $sql =
            "SELECT `type`, COUNT(*) AS count FROM pmp_temptable GROUP BY `type`";
        $res = dbexec($sql);
        if (mysql_num_rows($res) != 0) {
            ...
            if ($pmp_parser_mode == 1) {
                $sql =
                    "SELECT id FROM pmp_film WHERE id NOT IN (SELECT id FROM pmp_temptable)";
                $res = dbexec($sql);
                ...
                if ($profiles["deleted"] != 0) {
                    ...
                    foreach ($tables as $table) {
                        $sql =
                            "DELETE FROM " .
                            $table .
                            " WHERE id NOT IN (SELECT id FROM pmp_temptable)";
                        dbexec($sql);
                    }
                }
            }
    ...
    genTopTags();
    [$ids, $link, $indb, $notindb, $symlink, $windows] = getScreenshotsAdm(
        "../screenshots",
    );
    ...
    if ($pmp_gdlib == true && $pmp_thumbnail_cache == true) {
        ...
        while (($file = readdir($handle)) !== false) {
            ...
            if (
                (strrchr($file, ".") == ".jpg" &&
                    stristr($file, "b2evo_captcha") === false) ||
                strrchr($file, ".") == ".png"
            ) {
                ...
                elseif (
                    md5_file("../cover/" . $sourceFilename) !=
                    substr(
                        $file,
                        strpos($file, "_") + 1,
                        strrpos($file, "_") - strpos($file, "_") - 1,
                    )
                ) {
                    @unlink("../cache/" . basename($file));
                    $deleted++;
    ...
    if (isset($pmp_build_banners) && $pmp_build_banners != 0) {
        if (!buildBanners()) {
            $smarty->assign(
                "Error",
                "Something went wrong when building banner(s).",
            );
        }
    }
    ...
    if ($pmp_parser_mode == 0) {
        $smarty->assign("split", $_GET["splits"]);
    }
...
while (false !== ($file = readdir($handle))) {
    if (strrchr($file, ".") == ".xml") {
        ...
        $files[] = ["name" => $file, "size" => intval($size) . " " . $typ];
    }
}
...
if ($files) {
    ...
    $smarty->assign(
        "Info",
        t(
            "First of all you have to upload the exported XML-file to the xml directory. Also you can compress this file and upload a zip-file with the form of this page.",
        ),
    );
}
