...
class PDF extends FPDF
{
        ...
        if ($evenonpageone || $this->PageNo() != 1) {
            ...
            for ($i = 0; $i < count($this->header); $i++) {
                $this->Cell(
                    $this->colwidth[$i],
                    7,
                    $this->truncTT($this->header[$i], $this->colwidth[$i]),
                    1,
                    0,
                    "C",
                    1,
    ...
    function truncTT($string, $width, $suffix = "...")
    {
        ...
        if ($this->GetStringWidth($string) > $width) {
            if ($suffix != "") {
                return $this->truncTT(
                    mb_substr($string, 0, mb_strlen($string) - 2) . $suffix,
                    $width,
                    "",
                );
            } else {
                return $this->truncTT(
                    mb_substr($string, 0, mb_strlen($string) - 5) . "...",
                    $width,
                    "",
                );
            }
    ...
    function generateReport($header, $data)
    {
        ...
        foreach ($data as $row) {
            ...
            for ($i = 0; $i < count($this->colwidth); $i++) {
                $this->Cell(
                    $this->colwidth[$i],
                    5,
                    $this->truncTT($row[$i], $this->colwidth[$i]),
                    "LRB",
                    0,
                    "L",
                    1,
        ...
        $this->SetX(
            $this->w -
                $this->rMargin -
                $this->GetStringWidth(
                    t("Generated by ") . "phpMyProfiler V" . $pmp_version,
                ) -
                3,
        );
        ...
        $this->write(
            6,
            "phpMyProfiler " . $pmp_version,
            "http://www.genesworld.net",
        );
    }
}
...
if (isset($_GET["report"]) && $_GET["report"] == "pdf") {
    if (!empty($_POST["col1"])) {
        $header[0] = html_entity_decode(
            t(getColumns($_POST["col1"], "Header")),
        );
    }
    if (!empty($_POST["col2"])) {
        $header[1] = html_entity_decode(
            t(getColumns($_POST["col2"], "Header")),
        );
    }
    if (!empty($_POST["col3"])) {
        $header[2] = html_entity_decode(
            t(getColumns($_POST["col3"], "Header")),
        );
    }
    if (!empty($_POST["col4"])) {
        $header[3] = html_entity_decode(
            t(getColumns($_POST["col4"], "Header")),
        );
    }
    if (!empty($_POST["col5"])) {
        $header[4] = html_entity_decode(
            t(getColumns($_POST["col5"], "Header")),
        );
    }
    if (!empty($_POST["col6"])) {
        $header[5] = html_entity_decode(
            t(getColumns($_POST["col6"], "Header")),
        );
    }
} else {
    if (!empty($_POST["col1"])) {
        $header[0] = t(getColumns($_POST["col1"], "Header"));
    }
    if (!empty($_POST["col2"])) {
        $header[1] = t(getColumns($_POST["col2"], "Header"));
    }
    if (!empty($_POST["col3"])) {
        $header[2] = t(getColumns($_POST["col3"], "Header"));
    }
    if (!empty($_POST["col4"])) {
        $header[3] = t(getColumns($_POST["col4"], "Header"));
    }
    if (!empty($_POST["col5"])) {
        $header[4] = t(getColumns($_POST["col5"], "Header"));
    }
    if (!empty($_POST["col6"])) {
        $header[5] = t(getColumns($_POST["col6"], "Header"));
    }
}
...
if (!empty($header) && is_array($header)) {
    ...
    if (isset($_GET["usequery"]) && $_GET["usequery"] == "true") {
        ...
        } elseif ($_POST["where"] == "Ordered") {
            $sql .= ' WHERE collectiontype = \'Ordered\'';
        } elseif ($_POST["where"] == "WishList") {
    ...
    while ($row = mysql_fetch_object($res)) {
        ...
        if (!empty($_POST["col1"])) {
            $tmpdata[0] = $dvd->get(getColumns($_POST["col1"], $columnoutput));
        }
        if (!empty($_POST["col2"])) {
            $tmpdata[1] = $dvd->get(getColumns($_POST["col2"], $columnoutput));
        }
        if (!empty($_POST["col3"])) {
            $tmpdata[2] = $dvd->get(getColumns($_POST["col3"], $columnoutput));
        }
        if (!empty($_POST["col4"])) {
            $tmpdata[3] = $dvd->get(getColumns($_POST["col4"], $columnoutput));
        }
        if (!empty($_POST["col5"])) {
            $tmpdata[4] = $dvd->get(getColumns($_POST["col5"], $columnoutput));
        }
        if (!empty($_POST["col6"])) {
            $tmpdata[5] = $dvd->get(getColumns($_POST["col6"], $columnoutput));
        }
        unset($dvd);
