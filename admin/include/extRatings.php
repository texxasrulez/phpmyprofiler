...
function getIMDBID($site)
{
    ...
    if (
        preg_match(
            "@imdb.com/Title\?(\w*)\"\s*target=\"_blank\"@i",
            $ofdb_website,
            $match,
        )
    ) {
...
function getIMDBRating($site)
{
    if (
        preg_match(
            '~<span itemprop="ratingValue">(.*)</span>~Ui',
            $site,
            $match,
        )
    ) {
    ...
    if (
        preg_match(
            '~<span itemprop="ratingCount">(.*)</span>~Ui',
            $site,
            $match,
        )
    ) {
    ...
    return [$rating, $votes, $top, $bottom];
}
...
function getRottenRating($site)
{
    ...
    if (
        preg_match(
            '!Reviews Counted: <span itemprop="reviewCount">([\d\,]+)!i',
            $site,
            $match,
        )
    ) {
    ...
    return [$ratingCritics, $votesCritics, $ratingUser, $votesUser];
}
