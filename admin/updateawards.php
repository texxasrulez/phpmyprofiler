...
function startElement($parser, $name, $attrs)
{
    ...
    if ($name == "CATEGORY") {
        $category_i++;
        $movie_i = 0;
        $awards["CATEGORY"][$category_i] = ["NAME" => maskData($attrs["NAME"])];
    }
    if ($name == "MOVIE") {
        $movie_i++;
        $awards["CATEGORY"][$category_i]["MOVIE"][$movie_i] = [
            "TITLE" => maskData($attrs["NAME"]),
...
function writeAward()
{
    ...
    if (empty($del)) {
        $del = "DELETE FROM pmp_awards WHERE ";
    }
    $del .=
        "(award='" .
    ...
    for ($i = 1; $i <= count($awards["CATEGORY"]); $i++) {
        ...
        for ($p = 1; $p <= count($awards["CATEGORY"][$i]["MOVIE"]); $p++) {
            ...
            if (empty($insert)) {
                $insert = "INSERT INTO pmp_awards VALUES ";
            }
            $insert .=
                "('" .
                trim(
                    mysql_real_escape_string(
                        stripslashes(
                            $awards["CATEGORY"][$i]["MOVIE"][$p]["TITLE"],
                        ),
                    ),
                ) .
                "','" .
                ...
                "','" .
                trim(
                    mysql_real_escape_string($awards["CATEGORY"][$i]["NAME"]),
                ) .
                "','" .
                $win .
                "','" .
                trim(
                    mysql_real_escape_string(
                        getIsset(
                            $awards["CATEGORY"][$i]["MOVIE"][$p]["NOMINEE"],
                        ),
                    ),
                ) .
                "'),";
...
if (isset($_GET["action"]) && $_GET["action"] == "deleteaward") {
    ...
    if (isset($_GET["award"])) {
        ...
        // Delete existing awards
        $del =
            'DELETE FROM pmp_awards WHERE award = \'' .
            mysql_real_escape_string((string) $_GET["award"]) .
            '\';';
        dbexec($del, true);
    }
}
...
if (isset($_GET["action"]) && $_GET["action"] == "parse") {
    admin_require_post_csrf();
    if (isset($_GET["file"])) {
        $xml_file = basename((string) $_GET["file"]);
...
if (isset($_GET["action"]) && $_GET["action"] == "buildtags") {
    ...
    if (isset($_GET["award"])) {
        ...
        $del =
            "DELETE FROM pmp_tags WHERE name = '" .
            mysql_real_escape_string((string) $_GET["award"]) .
            "';";
        dbexec($del, true);
        ...
        $sql =
            "SELECT DISTINCT id FROM pmp_film LEFT JOIN pmp_awards ON LOWER(pmp_film.originaltitle) = LOWER(pmp_awards.title) WHERE award = '" .
            mysql_real_escape_string($_GET["award"]) .
            "' AND winner ='1' AND originaltitle != '' AND awardyear BETWEEN pmp_film.prodyear-1 AND pmp_film.prodyear+1;";
        $res = dbexec($sql);
        $count = mysql_num_rows($res);
        while ($row = mysql_fetch_object($res)) {
            if (empty($insert)) {
                $insert = "INSERT INTO pmp_tags VALUES ";
            }
            $insert .=
                "('" .
                $row->id .
        ...
        // Get winner by title
        $sql =
            "SELECT DISTINCT id FROM pmp_film LEFT JOIN pmp_awards ON LOWER(pmp_film.title) = LOWER(pmp_awards.title) WHERE award = '" .
            mysql_real_escape_string($_GET["award"]) .
            "' AND winner ='1' AND originaltitle = '' AND awardyear BETWEEN pmp_film.prodyear-1 AND pmp_film.prodyear+1;";
        $res = dbexec($sql);
        ...
        $smarty->assign(
            "Success",
            t("Tags table updated.") . " " . $count . " " . t("Tags created."),
        );
...
if ($files) {
    ...
    $smarty->assign(
        "Info",
        t(
            "First of all you have to upload an award file to the award directory.",
        ),
    );
}
