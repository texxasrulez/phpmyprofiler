...
function getTempMissing($file)
{
    ...
    foreach ($res[1] as $str) {
        $preg_str = preg_extract($str);
        $used_templates[
            "themes/" . $pmp_theme . "/templates/" . $preg_str
        ] = true;
...
function getTemp($file, $file_php = "0")
{
    ...
    if (file_exists($dir . $file)) {
        ...
        foreach ($temp_incl[1] as $str) {
            if (
                !strstr("header.tpl", $str) &&
                !strstr("footer.tpl", $str) &&
                !strstr("mainerror.tpl", $str) &&
                !empty($str) &&
                substr($str, 0, 1) != "{"
            ) {
                if (
                ...
                $used_templates[
                    "themes/" . $pmp_theme . "/templates/" . $preg_incl
                ] = true;
                ...
                if (
                    ...
                    foreach ($res_incl[1] as $str_incl) {
                        if (
                            !empty($str_incl) &&
                            substr($str_incl, 0, 1) != "{" &&
                            !isset($onlyone[$str_incl])
                        ) {
                            if (!empty($lang_data[$str_incl])) {
                                $templates[$file_php]["used"][][$str_incl] =
                                    $lang_data[$str_incl];
                                unset($unused[$str_incl]);
                                $onlyone[$str_incl] = true;
                            } else {
                                $templates[$file_php]["missing"][][
                                    $str_incl
                                ] = false;
                                $onlyone[$str_incl] = true;
                            }
...
foreach ($tdirs as $tdir) {
    ...
    while (false !== ($file = readdir($handle))) {
        if (
            is_file($tdir . "/" . $file) &&
            !isset($used_templates[substr($tdir, 3) . "/" . $file])
        ) {
