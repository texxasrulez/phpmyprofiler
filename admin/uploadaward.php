...
$smarty->assign("header", t("Upload Awards"));
$smarty->assign("header_img", "uploadcsv");
...
if (isset($_FILES["file"]) && $_FILES["file"]["name"] != "") {
    if (!is_writeable($uploadDir)) {
        $smarty->assign("Error", t("The directory awards is not writeable."));
    } else {
                        ...
                        if (uncompress_file($infilename)) {
                            if (!@unlink($infilename)) {
                                $smarty->assign(
                                    "Error",
                                    t(
                                        'Can\'t delete compressed collection file',
                                    ),
                                );
                            } else {
                                $smarty->assign(
                                    "Success",
                                    t("Added new award file."),
                                );
                            }
                        } else {
                            $smarty->assign(
                                "Error",
                                t("Unable to decompress award file."),
                            );
                            @unlink($_FILES["file"]["tmp_name"]);
                        }
                    } else {
                        $smarty->assign("Success", t("Added new award file."));
                    }
                } else {
...
$smarty->display("admin/uploadaward.tpl");
?>
