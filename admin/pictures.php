...
if (isset($_GET["action"]) && $_GET["action"] == "delete") {
    ...
    $picture_id = request_int($_GET, "id", 0, 1);
    if ($picture_id > 0) {
        $sql = "SELECT filename FROM pmp_pictures WHERE id = ?";
        $res = dbexec_prepared($sql, "i", [$picture_id]);
        $row = mysql_fetch_object($res);
        @unlink("../pictures/" . $row->filename);

        $sql = "DELETE FROM pmp_pictures WHERE id = ?";
        $res = dbexec_prepared($sql, "i", [$picture_id]);
...
if (isset($_GET["action"]) && $_GET["action"] == "add") {
    ...
    if (isset($_FILES["picture"]) && $_FILES["picture"]["name"] != "") {
        if (!is_writeable("../pictures/")) {
            $smarty->assign(
                "Error",
                t("The directory pictures is not writeable."),
            );
        } else {
                ...
                if ($ext !== false) {
                    ...
                    if (empty($_POST["filename"]) || strlen($name) < 5) {
                        if (strlen($name) < 5) {
                            $smarty->assign(
                                "Error",
                                t(
                                    "Filename is wrong and can't be used. New Filename generated.",
                                ),
                            );
                        }
                    ...
                    if (!file_exists("../pictures/" . $name)) {
                        if (
                            ...
                            $sql =
                                "INSERT INTO pmp_pictures (filename, title) VALUES (?, ?)";
                            ...
                            $smarty->assign(
                                "Success",
                                t("Added a new picture."),
                            );
                        } else {
                            $smarty->assign(
                                "Error",
                                t(
                                    "Can not move file from tmp into pictures dir.",
                                ),
                            );
                        }
                    } else {
