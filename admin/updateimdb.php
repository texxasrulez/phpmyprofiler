...
if (
    ...
    if (isset($wh)) {
        if ($wh == "old") {
            $sql .=
                " AND (to_days(lastupdate) < (to_days(now())-28) OR lastupdate = '0000-00-00')";
        } elseif ($wh == "new") {
    ...
    if ($maxids == 0) {
        ...
        if (isset($wh)) {
            header(
                "Location:updateimdb.php?action=update&what=" .
                    $db .
                    "_" .
                    $wh .
                    "&start=1&end=" .
...
if (
    ...
    if (isset($wh)) {
        if ($wh == "old") {
            $sql .=
                " AND (to_days(lastupdate) < (to_days(now())-28) OR lastupdate = '0000-00-00') LIMIT 0, " .
                ($endid - $startid);
        } elseif ($wh == "new") {
            $sql .=
                " AND lastupdate = '0000-00-00' LIMIT 0, " .
                ($endid - $startid);
        } elseif ($wh == "oldest") {
            $sql .= " ORDER BY lastupdate ASC LIMIT 0, 50";
        }
    ...
    $replace =
        "REPLACE INTO pmp_reviews_external (id,type,ext_id,review,votes,top250,bottom100,lastupdate) VALUES ";
...
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'imdb' AND (to_days(lastupdate) < (to_days(now())-28) OR lastupdate = '0000-00-00')";
$smarty->assign("imdb_old", mysql_num_rows(dbexec($sql)));
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'imdb' AND lastupdate = '0000-00-00'";
...
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'ofdb' AND (to_days(lastupdate) < (to_days(now())-28) OR lastupdate = '0000-00-00')";
$smarty->assign("ofdb_old", mysql_num_rows(dbexec($sql)));
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'ofdb' AND lastupdate = '0000-00-00'";
...
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'rotten_c' AND (to_days(lastupdate) < (to_days(now())-28) OR lastupdate = '0000-00-00')";
$smarty->assign("rotten_old", mysql_num_rows(dbexec($sql)));
$sql =
    "SELECT id FROM pmp_reviews_external WHERE type = 'rotten_c' AND lastupdate = '0000-00-00'";
