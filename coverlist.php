...
$query =
    'SELECT DISTINCT id FROM pmp_film WHERE collectiontype != \'Ordered\' AND collectiontype != \'Wish List\'
	  AND id NOT IN (SELECT id FROM pmp_tags where name = \'' .
    mysql_real_escape_string($pmp_exclude_tag) .
    '\') ORDER BY sorttitle LIMIT ' .
    ((int) $start - 1) * $pmp_cover_page .
    ", " .
    $pmp_cover_page;
$result = dbexec($query);
...
$query =
    'SELECT COUNT(id) AS num FROM pmp_film WHERE collectiontype != \'Ordered\' AND collectiontype != \'Wish List\' AND id NOT IN (SELECT id FROM pmp_tags where name = \'' .
    mysql_real_escape_string($pmp_exclude_tag) .
    '\')';
$row = dbexec($query);
...
$smarty->assign(
    "pages",
    (int) ($count / $pmp_cover_page + ($count % $pmp_cover_page == 0 ? 0 : 1)),
);
