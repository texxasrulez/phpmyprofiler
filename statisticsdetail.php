...
$report[$i]["sql"] =
    "SELECT YEAR(date) AS year, MONTH(date) AS month, MAX(id) AS visitors FROM pmp_counter GROUP BY MONTH(date), YEAR(date) ORDER BY date ASC";
...
$report[$i]["sql"] =
    "SELECT YEAR(date) AS year, MONTH(date) AS month, MAX(id) AS visitors FROM pmp_counter_profil GROUP BY MONTH(date), YEAR(date) ORDER BY date ASC";
$report[$i]["column"][] = "Year";
$report[$i]["column"][] = "Month";
$report[$i]["column"][] = "Visitors";
$report[$i]["translate"][] = "";
...
$report[$i]["sql"] =
    "SELECT prodyear, COUNT(id) AS count FROM pmp_film GROUP BY prodyear ORDER BY prodyear ASC";
$report[$i]["column"][] = "Year";
...
if ($pmp_statistic_showprice == true) {
    $report[$i]["sql"] =
        'SELECT YEAR(purchdate) AS year, MONTHNAME(purchdate) AS month, SUM(purchprice) AS price, COUNT(id) AS count FROM pmp_film WHERE collectiontype != \'Ordered\' AND collectiontype != \'Wish List\' AND year(purchdate) > 0 AND year(purchdate) != 1899 GROUP BY MONTH(purchdate), YEAR(purchdate) ORDER BY purchdate ASC';
} else {
    $report[$i]["sql"] =
        'SELECT YEAR(purchdate) AS year, MONTHNAME(purchdate) AS month, COUNT(id) AS count FROM pmp_film WHERE collectiontype != \'Ordered\' AND collectiontype != \'Wish List\' AND year(purchdate) > 0 AND year(purchdate) != 1899 GROUP BY MONTH(purchdate), YEAR(purchdate) ORDER BY purchdate ASC';
}
$report[$i]["column"][] = "Year";
$report[$i]["column"][] = "Month";
if ($pmp_statistic_showprice == true) {
...
$report_id = isset($_GET["id"]) ? (int) html2txt($_GET["id"]) : 1;
if (!isset($report[$report_id])) {
    $report_id = 1;
}

$result = dbexec($report[$report_id]["sql"]);

while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
    if (isset($report[$report_id]["replace"])) {
        $name = $row[0];
        $birthyear = $row[3];

        foreach ($report[$report_id]["replace"] as $col => $repl) {
            $row[$col] = str_replace(
                "[VALUE1]",
                rawurlencode($row[$col]),
                $repl,
            );
    ...
    if (isset($report[$report_id]["pic"])) {
        foreach ($report[$report_id]["pic"] as $col => $pic) {
            $picname = getHeadshot(
                $row[$col],
                $row[3],
                $row[4],
...
$smarty->assign("report", $report[$report_id]);
