...
function pmp_allowed_sort_fields()
{
    $allowed = [];
    foreach (getColumns() as $column) {
        if (!empty($column["Sort"]) && $column["Sort"] !== "Hide column") {
            $allowed[$column["Sort"]] = true;
        }
    }
...
function pmp_is_allowed_filter_field($field)
{
    $field = (string) $field;
    $simple = [
        "collectiontype" => true,
    ...
    return (bool) preg_match('/^pmp_[a-z0-9_]+\[dot\][a-z0-9_]+$/i', $field);
}

function pmp_is_allowed_join_value($value)
{
    return (bool) preg_match('/^[a-z0-9_]+\[dot\].+$/i', (string) $value);
}
...
function generateSql($count = false)
{
    ...
    $join[] =
        "LEFT JOIN pmp_boxset ON pmp_film.id = pmp_boxset.childid LEFT JOIN pmp_collection ON pmp_film.collectiontype = pmp_collection.collection";
    ...
    if (isset($_SESSION["letter"])) {
        if ($_SESSION["letter"] == "#") {
            ...
            $where[] =
                "pmp_film.sorttitle RLIKE '^" . $_SESSION["letter"] . "'";
        }
    }
    ...
    if (isset($_SESSION["list_where"])) {
        foreach ($_SESSION["list_where"] as $feld => $val) {
            ...
            if ($val["field"] == "media") {
                ...
                } elseif ($val["value"] == "HD DVD") {
                    $where[] = "pmp_film.media_hddvd = '1'";
                } elseif ($val["value"] == "Blu-ray") {
                    $where[] = "pmp_film.media_bluray = '1'";
                } elseif ($val["value"] != "") {
                    $where[] =
                        "pmp_film.media_custom = '" .
                        mysql_real_escape_string($val["value"]) .
                        "'";
                }
            } elseif ($val["field"] == "title") {
                $where[] =
                    "MATCH (title, originaltitle, sorttitle) AGAINST ('" .
                    mysql_real_escape_string($val["value"]) .
                    "')";
            } elseif (strpos($val["field"], "[dot]")) {
                $j++;
                ...
                $join[] =
                    "INNER JOIN " .
                    $tmp1[0] .
                    " t$j ON pmp_film.id = t$j." .
                    $tmp1[1];
                $where[] =
                    "t$j." .
                    $tmp2[0] .
                    " = '" .
                    mysql_real_escape_string($tmp2[1]) .
                    "'";
            } else {
                if (strpos($val["field"], "[tab]") === false) {
                    $val["field"] = "pmp_film." . $val["field"];
                } else {
                    $val["field"] = str_replace(
                        "[tab]",
                        "pmp_film.",
                        $val["field"],
                    );
                ...
                if (
                    strpos($val["value"], "_") !== false ||
                    strpos($val["value"], "%") !== false
                ) {
                    $where[] =
                        $val["field"] .
                        " like '" .
    ...
    if ($pmp_exclude_tag != "") {
        $where[] =
            "pmp_film.id NOT IN (SELECT id FROM pmp_tags where name = '" .
    ...
    if (!$count) {
        ...
        $sql =
            "SELECT COUNT(distinct pmp_film.id ) AS c FROM pmp_film " . $zwisql;
    }
    ...
    if (isset($_SESSION["list_orderby"])) {
        if (strpos($_SESSION["list_orderby"], "[tab]") === false) {
            $sql .=
                "ORDER BY pmp_film." .
                $_SESSION["list_orderby"] .
                " " .
                $_SESSION["list_orderdir"] .
                " ";
        } else {
            $sql .=
                "ORDER BY " .
                str_replace("[tab]", "pmp_film.", $_SESSION["list_orderby"]) .
                " " .
                $_SESSION["list_orderdir"] .
                " ";
        }
    ...
    if (!$count) {
        $sql .=
            "LIMIT " .
            ((int) $_SESSION["list_page"] - 1) * $pmp_dvd_menue .
            ", " .
            $pmp_dvd_menue .
            " ";
    }
...
if (!empty($_GET["orderby"])) {
    ...
    $allowed_sort_fields = pmp_allowed_sort_fields();
    if (isset($allowed_sort_fields[$orderby])) {
        $_SESSION["list_orderby"] = $orderby;
    }
...
// Delete filter
if (isset($_GET["delwhere"])) {
    if (!pmp_is_allowed_filter_field($_GET["delwhere"])) {
        $_GET["delwhere"] = "";
    }
    if (isset($_SESSION["list_where"])) {
        foreach ($_SESSION["list_where"] as $key => $val) {
            if ($val["field"] == $_GET["delwhere"]) {
                if (isset($_GET["whereval"])) {
                    if (
                        $val["value"] == $_GET["whereval"] ||
                        (!empty($_GET["delwhere"]) && !empty($_GET["addwhere"]))
                    ) {
...
if (!empty($_GET["addwhere"]) && isset($_GET["whereval"])) {
    if (!pmp_is_allowed_filter_field($_GET["addwhere"])) {
        $_GET["addwhere"] = "";
    }
    if (
        strpos((string) $_GET["addwhere"], "[dot]") !== false &&
        !pmp_is_allowed_join_value($_GET["whereval"])
    ) {
        $_GET["addwhere"] = "";
    }
    ...
    if (!$found) {
        $whereval = (string) $_GET["whereval"];
        if (strlen($whereval) > 255) {
            $whereval = substr($whereval, 0, 255);
        }
...
if (isset($_GET["list_page"])) {
    $_SESSION["list_page"] = request_int($_GET, "list_page", 1, 1);
}
// Which letter selected?
if (isset($_GET["letter"])) {
    if ($_GET["letter"] == "off") {
        $_SESSION["letter"] = null;
    } elseif ($_GET["letter"] == "0") {
        $_SESSION["letter"] = "#";
    } else {
        $letter = strtoupper(substr((string) $_GET["letter"], 0, 1));
        $_SESSION["letter"] = preg_match('/^[A-Z]$/', $letter) ? $letter : null;
    }
}
...
if (!$smarty->isCached("menue.tpl", crc32($sql))) {
    ...
    if ($res === false) {
        ...
        $smarty->assign(
            "Error",
            t(
                "An error occured during your query. To continue working all filters will be set to default values.",
            ),
        );
    }
    ...
    $smarty->assign(
        "pages",
        (int) ($dvdcount / $pmp_dvd_menue +
            ($dvdcount % $pmp_dvd_menue == 0 ? 0 : 1)),
    );
    if (isset($_SESSION["letter"])) {
        $smarty->assign("letter", $_SESSION["letter"]);
    }
    ...
    // Origins
    $res = dbexec(
        'SELECT DISTINCT country FROM pmp_countries_of_origin where country != \'\'',
    );
    $origin = [];
    while ($row = mysql_fetch_object($res)) {
        $origin[] = $row->country;
