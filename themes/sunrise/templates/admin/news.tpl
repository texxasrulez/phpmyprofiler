...
<div id="mainpanel">
    {if !isset($editadd)}
	<div align="center" style="text-align: center"><input type="button" value="{t}Add news{/t}" onclick="location.href='news.php?action=add&amp;{$csrf_token_url}';" /></div>
	...
	{if !isset($Info)}
		...
		{foreach from=$news item=new key=sch}
			...
			<td class="td{$sch%2}" valign="top" style="width: 150px">
			    <a class="button" href="news.php?action=edit&amp;id={$new->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/edit.gif" alt="{t}Edit{/t}" border="0" />{t}Edit{/t}</a>
			    <a class="button" href="news.php?action=delete&amp;id={$new->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/recyclebin.gif" alt="{t}Delete{/t}" border="0" />{t}Delete{/t}</a>
			</td>
		    </tr>
		{/foreach}
	    </table>
    ...
    {else}
	{if $editadd == 'add'}
	    <form enctype="multipart/form-data" method="post" action="news.php?action=addsave&amp;{$csrf_token_url}" name="formular" accept-charset="utf-8">
		<div align="center">
		    <table>
			<tr>
	...
	{if $editadd == 'edit'}
		...
		<form enctype="multipart/form-data" method="post" action="news.php?action=editsave&amp;id={$change->id}&amp;{$csrf_token_url}" accept-charset="utf-8">
		    <div align="center">
			<table>
			    <tr>
