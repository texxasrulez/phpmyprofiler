...
<div id="mainpanel">
	...
	<table class="tabelle" cellpadding="0" cellspacing="0" border="0">
		...
		{if $pending|@count > 1}
			...
			<a class="button" href="reviews.php?action=allactivate&amp;{$csrf_token_url}">{t}Activate all review entries{/t}</a>
		    </td>
		{/if}
	    </tr>
	    <tr>
		...
		<tr>
		    ...
		    <td class="td{$sch%2}" style="vertical-align: top">
			<a class="button" href="reviews.php?action=activate&amp;id={$review->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/action.png" alt="{t}Activate{/t}" border="0" />{t}Activate{/t}</a>
			<a class="button" href="reviews.php?action=delete&amp;id={$review->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/recyclebin.gif" alt="{t}Delete{/t}" border="0" />{t}Delete{/t}</a>
		    </td>
		</tr>
	    {/foreach}
	</table>
