...
<div id="mainpanel">
	...
	<table class="tabelle" cellpadding="0" cellspacing="0" border="0">
		...
		{if $pending|@count > 1}
			...
			<a class="button" href="guestbook.php?action=allactivate&amp;{$csrf_token_url}">{t}Activate all guestbook entries{/t}</a>
		    </td>
		{/if}
	    </tr>
	    <tr>
		...
		<tr>
		    ...
		    <td class="td{$sch%2}">
			<form action="guestbook.php?action=comment&amp;id={$gbentry->id}&amp;{$csrf_token_url}" method="post" style="visibility: inline" accept-charset="utf-8">
			    <textarea name="comment" cols="30" rows="2">{$gbentry->comment|stripslashes}</textarea><br />
			    <input type="submit" value="{t}OK{/t}" />
			</form>
		    </td>
		    <td class="td{$sch%2}" style="vertical-align: top">
			<a class="button" href="guestbook.php?action=activate&amp;id={$gbentry->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/action.png" alt="{t}Activate{/t}" border="0" />{t}Activate{/t}</a>
			<a class="button" href="guestbook.php?action=delete&amp;id={$gbentry->id}&amp;{$csrf_token_url}"><img src="../themes/{$pmp_theme}/images/recyclebin.gif" alt="{t}Delete{/t}" border="0" />{t}Delete{/t}</a>
		    </td>
		</tr>
	    {/foreach}
	</table>
