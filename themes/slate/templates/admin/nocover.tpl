...
<div id="mainpanel">
	...
	{foreach from=$dvds item=dvd key=sch}
		...
		<td class="td{$sch%2}_over" style="text-align: center">
		    ...
		    {else}
			<a onclick="javascript:window.open('getcover.php?cover={$dvd->id}f&amp;nohead=true&amp;{$csrf_token_url}', 'getcover','resizable=1,location=0,directories=0,status=0,menubar=0,scrollbars=0,toolbar=0,width=450,height=400');return false;" href="getcover.php?cover={$dvd->id}f&amp;{$csrf_token_url}">{$dvd->frontpic|getpic:boolean}</a>
		    {/if}
		</td>
		<td class="td{$sch%2}_over" style="text-align: center">
		  {if $dvd->backpic}
		  ...
		  {else}
		      <a onclick="javascript:window.open('getcover.php?cover={$dvd->id}b&amp;nohead=true&amp;{$csrf_token_url}', 'getcover','resizable=1,location=0,directories=0,status=0,menubar=0,scrollbars=0,toolbar=0,width=450,height=400');return false;" href="getcover.php?cover={$dvd->id}b&amp;{$csrf_token_url}">{$dvd->backpic|getpic:boolean}</a>
		  {/if}
		</td>
	    </tr>
	{/foreach}
