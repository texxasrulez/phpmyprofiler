    ...
    if (!isset($_POST["form_key"]) || !$formKey->validate()) {
        ...
        if (count($msg) == 0) {
            ...
            elseif (!empty($_POST["username"])) {
                ...
                // Insert enty into db
                $query = 'INSERT INTO pmp_guestbook (date, name, email, text, status, url)
					VALUES (now(), ?, ?, ?, ?, ?)';

                if (
                    dbexec_prepared($query, "sssis", [
                        $name,
                        $email,
                        $message,
                        (int) $pmp_guestbook_activatenew,
                        $url,
                    ])
                ) {
                    ...
                    $subject =
                        "[phpMyProfiler] " .
                        t("%name added a new guestbook entry", [
                            "%name" => $name,
                        ]);
                    ...
                    $smarty->assign(
                        "Success",
                        t("Thank you. The guest book entry was stored."),
                    );
                    ...
                    $smarty->assign(
                        "Error",
                        t(
                            "When storing in the database unfortunately an error occurs.",
                        ),
                    );
                }
            }
...
// Page selected?
$start = request_int($_GET, "page", 1, 1);
...
$query =
    'SELECT name, email, date_format(date, \'' .
    $pmp_dateformat .
    '\') AS date, text, url, comment
	  FROM pmp_guestbook WHERE status != 0 ORDER BY id DESC LIMIT ' .
    ((int) $start - 1) * $pmp_entries_side .
    ", " .
    $pmp_entries_side;
$result = dbexec($query);
...
$smarty->assign(
    "pages",
    (int) ($count / $pmp_entries_side +
        ($count % $pmp_entries_side == 0 ? 0 : 1)),
);
