...
// News
$sql =
    "SELECT id, date, title, text FROM pmp_news ORDER BY date DESC LIMIT 0, 1";
$res = dbexec($sql);
...
// Last DVDs
$sql =
    'SELECT pmp_film.id FROM pmp_film
...
// Number of DVDs
$query =
    'SELECT SUM(countas) AS count FROM pmp_film
	WHERE collectiontype != \'Ordered\' AND collectiontype != \'Wish List\'
	AND pmp_film.id NOT IN (SELECT id FROM pmp_tags where name = \'' .
    mysql_real_escape_string($pmp_exclude_tag) .
    '\')';
...
if (mysql_num_rows($result) > 0) {
    $smarty->assign(
        "last_update",
        strftime($pmp_dateformat, strtotime(mysql_result($result, 0, "data"))),
    );
}
