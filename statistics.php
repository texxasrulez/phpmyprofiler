...
if ($pmp_statistic_showprice == true) {
    ...
    $expensive = [];
    $query =
        'SELECT value FROM pmp_statistics WHERE type = \'highest_price\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
    ...
    $cheapest = [];
    $query =
        'SELECT value FROM pmp_statistics WHERE type = \'lowest_price\' ORDER BY LPAD(data, 20, \'0\') ASC';
    $result = dbexec($query);
    while ($row = mysql_fetch_object($result)) {
...
$long = [];
$query =
    'SELECT value FROM pmp_statistics WHERE type = \'longest\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
...
$short = [];
$query =
    'SELECT value FROM pmp_statistics WHERE type = \'shortest\' ORDER BY LPAD(data, 20, \'0\') ASC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
...
$rating = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'ratings\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $rating[] = ["name" => $row->name, "data" => $row->data];
}
...
$date = [];
$query =
    'SELECT name, value, data FROM pmp_statistics WHERE type = \'purchdate\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $date[] = $row;
}
...
$year = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'decade\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $year[] = $row;
}
...
$place = [];
$query =
    'SELECT name, value, data FROM pmp_statistics WHERE type = \'purchplace_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $place[] = $row;
}
...
$genre = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'genres_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $genre[] = $row;
}
...
$studio = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'studios_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $studio[] = $row;
}
...
$companies = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'company_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $companies[] = $row;
}
...
$region = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'regions\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $region[] = $row;
}
...
$origin = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'origins\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $origin[] = $row;
}
...
$locality = [];
$query =
    'SELECT name, data FROM pmp_statistics WHERE type = \'localities\' ORDER BY LPAD(data, 20, \'0\') DESC';
$result = dbexec($query);
while ($row = mysql_fetch_object($result)) {
    $locality[] = $row;
}
